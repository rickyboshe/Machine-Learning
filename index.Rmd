...
---
<center>
# NYC school’s academic performance vs public perception
</center>

## A Data Visualisation Approach
...
I used data visualisation to explore:
...
fig2<-ggplotly(fig2)%>%layout(margin = list(l = 70, r = 30, b = 20, 
                                            t = 80),
...
```{r perform, include=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
...
fig3<-ggplotly(fig3)%>%layout(margin = list(l = 70, r = 30, b = 20, 
                                            t = 80),
           yaxis = list(title = paste0(c(rep("&nbsp;", 10),
                                         "Average SAT score",
                                         rep("&nbsp;", 8),
...
plot1<-plotdata%>%
  ...
  #geom_text(aes(label = lbl), size = 1.5, 
...
#api_create(plot1, filename = "Percentage of schools with at least a third of students in special programs")
plot1
```
...
fig4<-ggplotly(fig4)#%>%config(responsive=TRUE, displayModeBar = FALSE)
#api_create(fig4, filename = "Correlation between school \nperformance and demographics")
fig4
...
#api_create(fig5, filename = "Perception of NYC schools quality (survey responses)")
fig5
```
...
#api_create(fig6, filename = "NYC School quality perception survey questions and responses")
fig6
...
fig7<-ggplotly(fig7)#%>%config(boro, displayModeBar = FALSE)
#api_create(boro, filename = "Perception of NYC schools quality by Borough (survey responses)")
fig7
```
